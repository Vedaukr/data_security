...
# used https://lzutao.github.io/cribdrag/
found_part = "if you can talk with crowds and keep your virtue"
found_str_index = 8

def apply_xor(str1, str2):
    return bytes([x ^ y for x, y in zip(str1, str2)])

key = apply_xor(bytes(found_part, encoding='ascii'), bytes.fromhex(messages[found_str_index]))

# authored by Rudyard Kipling
for msg in messages:
    encoded = apply_xor(key, bytes.fromhex(msg))
    print(encoded.decode(encoding='ascii', errors='ignore'))